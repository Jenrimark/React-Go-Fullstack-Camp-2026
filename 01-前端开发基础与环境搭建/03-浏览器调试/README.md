...
## [“资源（Sources）”面板](#zi-yuan-sources-mian-ban)
...
![](assets/01-chrome-open-sources.svg)

切换按钮 [图标] 会打开文件列表的选项卡。

让我们在预览树中点击和选择 `hello.js`。这里应该会如下图所示：

![](assets/02-chrome-tabs.svg)
...
现在你可以再次点击切换按钮 [图标] 隐藏资源列表来给代码腾出一些空间。

## [控制台（Console）](#kong-zhi-tai-console)
...
![](assets/03-chrome-sources-console.svg)

## [断点（Breakpoints）](#duan-dian-breakpoints)
...
![](assets/04-chrome-sources-breakpoint.svg)
...
## [“debugger” 命令](#debugger-ming-ling)
...
![](assets/05-chrome-sources-debugger-pause.svg)
...
[图标] —— “步出（Step out）”：继续执行到当前函数的末尾，快捷键 `Shift+F11`。  
继续执行当前函数内的剩余代码，并暂停在调用当前函数的下一行代码处。当我们使用 [图标] 偶然地进入到一个嵌套调用，但是我们又对这个函数不感兴趣时，我们想要尽可能的继续执行到最后的时候是非常方便的。

[图标] —— 启用/禁用所有的断点。  
这个按钮不会影响程序的执行。只是一个批量操作断点的开/关。

[图标] —— 启用/禁用出现错误时自动暂停脚本执行。  
...
1.  断点。
2.  `debugger` 语句。
3.  error（如果开发者工具是打开状态，并且按钮 [图标] 是开启的状态）。
